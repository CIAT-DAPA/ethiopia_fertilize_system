...
# souring data preparation script
  ...
  n <- raster("wheat_n_above.tif")
  p <- raster("wheat_p_above.tif")
  k <- raster("wheat_k_above.tif")
  # s <- raster("wheat_S_potential.tif")
  yield_pot <- raster("wheat_optimal_yield_above.tif")
  
# ------------------------------------------------------------------------------ 
# stack rasters
  cov <- stack(stack_above, n,p, k, yield_pot)
...
# extracting values by points
  ...
  writeRaster(
    pred_90,
    filename = "wheat_attainable_yield_above",
    format = "GTiff",
    overwrite = TRUE)
  ...
  write.csv(csv_attainable, file = "wheat_attainable_yield_above.csv", row.names = F, sep = ",")
