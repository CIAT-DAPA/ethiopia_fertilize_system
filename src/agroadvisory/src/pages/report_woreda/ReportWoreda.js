...
function Report() {
    ...
    React.useEffect(async () => {
        ...
        if (reportInput.woreda) {
            GeoFeatures.geojsonWoreda("'" + reportInput.woreda[1] + "'").then((data_geo) => {
            ...
            let kebeles, ids = ''
            const values = [], suma = [];
            await axios.get(Configuration.get_url_api_base() + "adm4/" + reportInput.woreda[0])
                .then(response => {
            ...
            await kebeles.map(async (dato, index) => {
                ...
                ids += `${dato.id},`
                await axios.get(Configuration.get_url_api_base() + "metrics/" + dato.id)
                    .then(response => {
                        values.push(response.data)
                        response.data.map((datito, i) => {
                            if (suma[i]) {
                                suma[i].values[0].values[0] += (datito.values[0].values[0] / kebeles.length)
                                suma[i].values[1][0].values[0] += (datito.values[1][0].values[0] / kebeles.length)
                                suma[i].values[2][0].values[0] += (datito.values[2][0].values[0] / kebeles.length)
                            } else {
    ...
    const BarChartFert = ({ name, data }) => {
        return (
            <div className='card col-12 col-md-5 my-1' key={"bar_chart_" + name} style={{ minWidth: '49%' }}>
                <div className="card-body">
    ...
    const BarChartYield = ({ name, data }) => {
        return (
            <div className='card col-12 col-md-5 my-1' key={"bar_chart_yield"} style={{ minWidth: '49%' }}>
                <div className="card-body">
                    <h5 className="card-title">{name}</h5>
    ...
    const Spinners = () => {
        return (
            <div className='col-12 d-flex justify-content-evenly m-4' style={{ backgroundColor: "white" }} key={"spinners"}>
        ...
        <main>
            ...
            <section className='container'>
                <div className="d-flex font-link">
                    <h3>Woreda report: <b>{reportInput.woreda[1]}</b> </h3>
                </div>
                <div>
